...
conv5.addEventListener('mouseover', function(e) {
  const buttonId = e.currentTarget.id;
  currency(buttonId);
})
...
function currency(id) {
  ...
  .then(data => {
    ...
    const chf = all['CHF']
    if (id === conv[0].id) {
      conv[0].innerText = aud;
    }
    if (id === conv[1].id) {
      conv[1].innerText = bgn;
    }
    if (id === conv[2].id) {
      conv[2].innerText = brl;
    }
    if (id === conv[3].id) {
      conv[3].innerText = cad;
    }
    if( id ===conv[4].id) {
      conv[4].innerText = chf;
    }
    })
